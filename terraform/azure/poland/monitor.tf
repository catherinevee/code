resource "azurerm_cost_anomaly_alert" "poland-costanomalyalert" {
  name            = "${var.defaultlocation}_costanomalyalert"
  display_name    = "${var.defaultlocation}_costanomalyalert"
  #subscription_id = 
...
resource "azurerm_monitor_action_group" "poland-monitoractiongroup" {
  name                = "${var.defaultlocation}_monitoractiongroup"
  resource_group_name = "${var.defaultrg}"
  short_name          = "${var.defaultlocation}_monitoractiongroup"
  email_receiver {
...
data "azurerm_storage_account" "tfstate_sa" {
  ...
  resource_group_name = "${var.tfstate_storageaccount_location}"
}

resource "azurerm_monitor_metric_alert" "poland-monitormetricalert" {
  name                = "${var.defaultlocation}_metricalert"
  resource_group_name = "${var.defaultrg}"
